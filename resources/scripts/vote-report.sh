...
if [ -z "$EXPLORER_NODE" ]; then
    EXPLORER_NODE="https://wallets.cauri.cm/api"
fi
...
JsonData1=$( curl -s "$EXPLORER_NODE/delegates?page=1&limit=101" )
JsonData2=$( curl -s "$EXPLORER_NODE/delegates?limit=29&offset=101" )
...
echo '===================================================================' >> $WorkFile
echo '| Rank | Delegate                  | Vote % |  Vote CAU  | Voters |' >> $WorkFile
echo '===================================================================' >> $WorkFile
...
echo -e "\n $Date UTC / TxtVoteReport.sh v2.5.1 / cauri.cm \n" >> $WorkFile
echo -e "\n NOTE: This report is based on the 101 active delegates only! \n" >> $WorkFile
